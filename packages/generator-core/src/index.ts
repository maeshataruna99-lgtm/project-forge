...
function readTemplate(
  templatePack: "typescript-nest-vue" | "php-laravel",
  path: string,
): string {
  try {
...
`
  ...
  const integrationImports = [
    ...(config.features.apiDocs
      ? ["import { configureApiDocs } from './api-docs/setup';"]
      : []),
  ...
  const integrationModules = [
    ...(config.features.redis ? ["RedisModule"] : []),
    ...(config.features.queue ? ["QueueModule"] : []),
    ...(config.features.realtime ? ["RealtimeModule"] : []),
    ...(config.features.smtp ? ["EmailModule"] : []),
...
`
  ...
  const ecommerceAdminGrants = ecommerce
    ? ", 'products:read', 'products:manage'"
    : "";
  const ecommerceSeedPermissions = ecommerce
    ? "[['products:read', 'Read product catalog'], ['products:manage', 'Manage products']]"
    : "[]";
  const ecommerceSeedNavigation = ecommerce
    ? "[['products', 'Products', '/products', 'products:read', 15]]"
    : "[]";
  const optionalReadme = [
    ...(config.features.redis
      ? [
          "Redis connects through REDIS_URL. Queue jobs are stored in a Redis list; a worker/consumer still needs to be implemented for processing.",
        ]
      : []),
    ...(config.features.realtime
      ? [
          "Realtime exposes a small WebSocket ping/pong example. Set WEB_ORIGIN to the allowed browser origin in production.",
        ]
      : []),
    ...(config.features.apiDocs
      ? [
          "OpenAPI docs are available at /docs. Review and restrict the endpoint before exposing sensitive APIs publicly.",
        ]
      : []),
    ...(config.features.smtp
      ? [
          "SMTP sends text email only after SMTP_HOST and the optional credentials are configured.",
        ]
      : []),
    ...(config.features.uploads
      ? [
          "Uploads keeps validated PNG, JPEG, and PDF files in memory for the request only; no permanent storage is configured.",
        ]
      : []),
    ...(config.features.rateLimit
      ? [
          "The API uses an in-memory per-process rate limit; use a shared store before horizontally scaling.",
        ]
      : []),
    ...(config.features.logging
      ? [
          "The structured logger redacts common credential fields from string and JSON messages.",
        ]
      : []),
    ...(config.features.generatedTests
      ? ["A generated feature test is included with the API test suite."]
      : []),
    ...(config.features.ciCd
      ? [
          "GitHub Actions runs install, build, and test checks for pushes and pull requests.",
        ]
      : []),
  ];
  ...
  const dockerReadme =
    config.features.docker || config.deploymentProfile === "docker"
...
`;
  const authFrontendImport = auth
    ? "import StarterAuth from './components/StarterAuth.vue';"
    : "";
...
`
  ...
  return source
    .replaceAll(
      "project-forge/laravel-api-starter",
      `project-forge/${config.project.name}`,
    )
    .replaceAll("__PROJECT_NAME__", config.project.name)
    .replaceAll("__UI_LAYOUT__", config.ui.layout)
    .replaceAll(
      "Blank fullstack starter",
      ecommerce ? "E-commerce fullstack starter" : "Blank fullstack starter",
    )
    ...
    .replaceAll(
      "__PROFILE_LABEL__",
      config.project.profile === "enterprise" ? "Enterprise" : "Minimal",
    )
    .replaceAll(
    ...
    .replaceAll(
      "/*__INTEGRATION_MODULES__*/",
      integrationModules ? `, ${integrationModules}` : "",
    )
    .replaceAll(
      "/*__API_DOCS_BOOTSTRAP__*/",
      config.features.apiDocs ? "  configureApiDocs(app);" : "",
    )
    .replaceAll(
      "/*__LOGGER_BOOTSTRAP__*/",
      config.features.logging ? "  app.useLogger(new StructuredLogger());" : "",
    )
    .replaceAll(
      "/*__RATE_LIMIT_BOOTSTRAP__*/",
      config.features.rateLimit
        ? "  app.useGlobalGuards(new ApiRateLimitGuard());"
        : "",
    )
    .replaceAll(
      "/*__API_DOCS_IMPORT__*/",
    ...
    .replaceAll(
      "__OPTIONAL_API_DEPENDENCIES__",
      optionalDependencies.length ? `, ${optionalDependencies.join(", ")}` : "",
    )
    .replaceAll(
      "__OPTIONAL_API_DEV_DEPENDENCIES__",
      optionalDevDependencies.length
        ? `, ${optionalDevDependencies.join(", ")}`
        : "",
    )
    ...
    .replaceAll(
      "__ECOMMERCE_MEMBER_CODES__",
      ecommerce ? "['products:read']" : "[]",
    )
    .replaceAll(
      "__ECOMMERCE_NAVIGATION_COUNT__",
      ecommerce && config.features.navigation === "dynamic" ? "5" : "4",
    )
    .replaceAll(
      "__ECOMMERCE_MEMBER_PRODUCT_ACCESS__",
      ecommerce ? "true" : "false",
    )
    .replaceAll(
      "__ECOMMERCE_ADMIN_PRODUCT_ACCESS__",
      ecommerce ? "true" : "false",
    )
    ...
    .replaceAll(
      "  /*__AUTH_COMPANY_RELATION__*/",
      auth ? "  memberships CompanyMembership[]" : "",
    )
    .replaceAll(
      "/*__AUTH_PRISMA_SCHEMA__*/",
      `${authSchema}${rbacSchema}${navigationSchema}${auditSchema}`,
    )
    .replaceAll(
      "  /*__AUDIT_COMPANY_RELATION__*/",
      config.features.audit ? "  auditEvents AuditEvent[]" : "",
    )
...
function prepareFiles(input: unknown): Record<string, Uint8Array> {
  ...
  const sources = new Map(
    [...baseFiles, ...featureFiles].map((file) => [
      file.destination,
      file.source,
    ]),
  );
  ...
  for (const path of plan.files) {
    ...
    const content = strToU8(
      render(readTemplate(plan.templatePack, sourcePath), config),
    );
...
export function createArchive(input: unknown): Uint8Array {
  const archive = zipSync(prepareFiles(input), {
    level: 6,
    mtime: ARCHIVE_MTIME,
  });
...
export function createArchiveAsync(input: unknown): Promise<Uint8Array> {
  ...
  return new Promise((resolveArchive, rejectArchive) => {
    const worker = new Worker(
      resolve(dirname(fileURLToPath(import.meta.url)), "zip-worker.cjs"),
    );
    let settled = false;
    const finish = (archive?: Uint8Array) => {
      ...
      if (!archive || archive.length > MAX_ARCHIVE_BYTES)
        rejectArchive(new GenerationError());
      else resolveArchive(archive);
    };
    const timeout = setTimeout(() => finish(), MAX_GENERATION_MS);
    worker.once("message", (message: { archive?: Uint8Array }) =>
      finish(message.archive),
    );
