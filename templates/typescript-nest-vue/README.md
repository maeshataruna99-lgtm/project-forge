# __PROJECT_NAME__

Blank fullstack starter generated by Project Forge. This explicitly minimal profile includes a NestJS API, Vue 3 frontend, PostgreSQL/Prisma schema, and theme tokens. It does not include authentication, RBAC, audit, or multi-company access controls. Add those before exposing business data to untrusted users.

## Requirements

- Node.js 22.12 or newer
- pnpm 9
- PostgreSQL for migrations and database-backed features

## Setup

1. Copy `.env.example` to `.env` and enter your own PostgreSQL connection values.
2. Run `pnpm install`.
3. Run `pnpm db:generate`.
4. Run `pnpm db:migrate` against a development PostgreSQL database.
5. Run `pnpm dev` to start API and frontend. API health: `http://localhost:3000/health`; frontend: `http://localhost:5173`.

`db:migrate` uses Prisma's development migration command and needs a running PostgreSQL service. The health route and frontend can run without a database connection, but database features need the migration first.

## Checks

```sh
pnpm build
pnpm test
```

The Prisma schema provides `Company` and `ExampleItem` as starting models. The API health route is the only API endpoint in this minimal starter.
The Vue app reads `/api/health` through Vite's development proxy. Configure the same `/api` reverse proxy when deploying the built frontend.
