...
export function createWizardState(storage: Storage) {
  ...
  function update(path: string, value: string): boolean {
    ...
    const nextValue =
      section === "features" && key !== "navigation" ? value === "true" : value;
    const result = projectConfigSchema.safeParse({
      ...config.value,
      [section]: { ...original, [key]: nextValue },
    });
  ...
  function next() {
    if (current.value < LAST_STEP) current.value++;
  }
  function back() {
    if (current.value > 0) current.value--;
  }
  function navigate(index: number) {
    if (index >= 0 && index < current.value) current.value = index;
  }
  function reset() {
    clearDraft(storage);
