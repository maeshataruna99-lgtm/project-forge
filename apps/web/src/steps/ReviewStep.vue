...
const props = defineProps<{
  ...
  githubState: 'idle' | 'authorizing' | 'awaiting' | 'authorized' | 'uploading' | 'complete' | 'error';
  githubAuthorization?: GitHubAuthorization;
  githubMessage?: string;
  githubRepositoryUrl?: string;
}>();
const emit = defineEmits<{ validate: []; download: []; githubToggle: [enabled: boolean]; githubAuthorize: []; githubCreate: [owner: string]; githubCancel: [] }>();
...
<template>
  <section aria-labelledby="review-heading">
    ...
    <fieldset class="github-output">
      ...
      <label><input type="checkbox" :checked="config.output.destination === 'github'" @change="emit('githubToggle', ($event.target as HTMLInputElement).checked)"> Create a public GitHub repository too</label>
      <p>ZIP download stays available. The public_repo permission covers public repositories your GitHub account can access. The API keeps the access token only in memory and discards it after one operation or expiry; GitHub may keep this app authorized until you revoke it in GitHub settings.</p>
      <button v-if="config.output.destination === 'github' && (githubState === 'idle' || githubState === 'error')" type="button" class="button-secondary" :disabled="!plan || validating" @click="emit('githubAuthorize')">{{ githubState === 'error' ? 'Reconnect GitHub' : 'Connect GitHub' }}</button>
      <p v-else-if="githubState === 'authorizing'" role="status">Starting GitHub authorization…</p>
      <div v-else-if="githubState === 'awaiting' && githubAuthorization" class="github-device-code">
        <p>Open <a :href="githubAuthorization.verificationUri" target="_blank" rel="noreferrer">GitHub device authorization</a> and enter this code:</p>
        <strong>{{ githubAuthorization.userCode }}</strong>
        <p role="status">Waiting for GitHub approval…</p>
        <button type="button" class="button-secondary" @click="emit('githubCancel')">Cancel authorization</button>
      </div>
      <div v-else-if="githubState === 'authorized'" class="github-create">
        <label for="github-owner">GitHub username or organization</label>
        <input id="github-owner" v-model="owner" autocomplete="off" required minlength="1" maxlength="39" pattern="[A-Za-z0-9](?:[A-Za-z0-9-]*[A-Za-z0-9])?">
        <p>Repository name: <strong>{{ config.project.name }}</strong>. Confirming below creates a public repository and pushes the generated files.</p>
        <button type="button" class="button-primary" :disabled="!plan || !owner.trim()" @click="emit('githubCreate', owner.trim())">Create public repository and push files</button>
      </div>
      <p v-else-if="githubState === 'uploading'" role="status">Creating the repository and pushing project files…</p>
      <p v-if="githubMessage" role="alert" class="field-error">{{ githubMessage }}</p>
      <p v-if="githubRepositoryUrl" role="status"><a :href="githubRepositoryUrl" target="_blank" rel="noreferrer">{{ githubRepositoryUrl }}</a></p>
      <p v-if="githubState === 'complete'" role="status">Project files were pushed successfully.</p>
    </fieldset>
